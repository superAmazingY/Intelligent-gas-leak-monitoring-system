...
#define PROID			"CFcMIQYDRa"

#define ACCESS_KEY		"Q0I1UmRWSktRWG5hcDNmYnNpRGNmYUZDUFpLbXZRc3I="
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	strcat(buf, text);
//	UsartPrintf(USART_DEBUG, "Beep_Status:%s\r\n",beep_info.Beep_Status? "true" : "false");

	memset(text, 0, sizeof(text));
	sprintf(text, "\"CH4\":{\"value\":%d},", MQ4.CH4);
	strcat(buf, text);
//  UsartPrintf(USART_DEBUG, "onenet CH4:%d\r\n",MQ4.CH4);delay_ms(10);
  memset(text, 0, sizeof(text));
	sprintf(text, "\"CO\":{\"value\":%d},", MQ7.CO);
	strcat(buf, text);
	
  memset(text, 0, sizeof(text));
	sprintf(text, "\"CO2\":{\"value\":%d},", JW_Value);
	strcat(buf, text);
